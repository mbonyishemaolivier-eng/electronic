...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['image']['name'])) {
        try {
            ...
            if ($uploaded) {
                $oldPath = __DIR__ . '/../assets/images/products/' . $product['image'];
                if ($product['image'] !== 'default.jpg' && file_exists($oldPath)) {
                    @unlink($oldPath);
                }
                $image = $uploaded;
            }
    ...
    if (empty($errors)) {
        $slug = uniqueProductSlug($db, generateSlug($name), $productId);
        ...
        logActivity($db, 'product_updated', 'Updated product: ' . $name, 'product', $productId);
    ...
    <div class="admin-card-body">
        <form method="POST" enctype="multipart/form-data" class="admin-form">
            ...
            <div class="current-image">
                ...
                <img src="../<?= getProductImage($product['image']) ?>" alt="Current product image" class="preview-image">
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="stock" name="stock" required min="0" value="<?= sanitize($_POST['stock'] ?? '') ?>">
                </div>
            </div>
